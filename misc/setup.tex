...
%----------------------------------------------------------------------------------------
% Two marks name the template this document was built from. Both are here so that
% they sit in one place, and both are meant to be findable: remove either one if you
% do not want it in your submission.
%
% 1. The PDF's creator field, the conventional place for a "made with" note. It is
%    not part of the text of the document, so nothing extracts it into your prose.
\hypersetup{pdfcreator={LaTeX with the TU Berlin MLSec thesis template,
                        https://github.com/jlb-jlb/tuberlin-mlsec-thesis-template}}
%
% 2. A line for the page, white on white inside a zero-size box, so it prints
%    nothing and shifts no layout. It is NOT hidden from software: copy and paste,
%    screen readers and text extraction all return it. NOTHING CALLS IT: the macro
%    is defined here and left unused, so no document carries the line unless you
%    place \templateattribution yourself. The verso of the title page, the colophon
%    position, is where it belongs if you want it.
\newcommand{\templateattribution}{%
  \smash{\makebox[0pt][c]{\textcolor{white}{\tiny Typeset with the TU Berlin MLSec
  thesis template, https://github.com/jlb-jlb/tuberlin-mlsec-thesis-template}}}%
}
